...
**Good Abstraction:** A great example is Python's built-in `len()` function. You use `len("abc")` for strings, `len([1, 2, 3])` for lists, and `len({'key': 'value'})` for dictionaries. Although these data types are different internally, `len()` provides one simple command to get the "size" or "number of items". It hides the specific details of *how* counting works for each type, and this works because these data types are all similar in the right way: they're all straightforwardly countable things. `len()` makes your code simpler and easier to write by grouping related operations together, and that's what makes it a good abstraction.

**Bad Abstraction:** `lm-evaluation-harness` often creates bad abstractions. It tries to force fundamentally different operations — like loading data, formatting a prompt, filtering output, and calculating a score — into a single, rigid configuration system (like its complex YAML files). Because these tasks aren't truly similar, the abstraction doesn't simplify anything. Instead, it leads to confusing workarounds (like embedding code snippets or using special `!function` tags) and ultimately makes the whole process *more* complex and harder to understand. This is bad abstraction: it adds complexity instead of hiding it.
