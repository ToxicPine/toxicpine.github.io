...
The productivity leak comes from the first kind being hidden. If an analyst uses AI to draft a report in twenty minutes instead of eight hours, they face a choice: disclose the gain and be rewarded with more work for the same pay, or keep quiet and enjoy the rest of the day. If working faster just gets you more work, you're not going to work faster. So you get workers holding four jobs simultaneously because ChatGPT handles most of the output, writing Slack messages to their managers in all lowercase so they look more organic.

If the bot can do half your job, what were you being paid for all this time? Workers hide the automation and pretend the slide deck took all night, because the grind is the proof that the work was hard, and hard work is what justifies the salary. The firm pays for the experiments in salary and throws away the results. One worker got fired after his manager noticed his emails had become too perfect. The lesson everyone else absorbed: if you're using AI, leave in some mistakes.
...
The second kind of automation is blocked for a different reason: it requires access workers shouldn't have. You can use AI to draft a report, but you can't have code directly update the customer database, because that means reading sensitive records and writing to production systems. To go from AI helping with the grunt work to code that acts on your behalf, workers would have to give their tools access they aren't authorized to grant. Samsung lifted its ChatGPT ban for the semiconductor division. Within three weeks, chip-testing source code was sitting on OpenAI's servers.

For twenty years, SaaS was defined by standardization. Because software was expensive and specialized, everyone bought the same tools: Salesforce, Workday, ServiceNow. If the standard choice protects you from blame and the non-standard choice doesn't, you're picking the standard choice. So everyone picks the same tools, and everyone's operations end up looking the same. You can't out-execute a competitor who runs the exact same stack you do. You're both constrained by the same vendor's assumptions about how businesses should work.
